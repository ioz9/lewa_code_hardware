		...
		public void handleMessage(Message msg) {
			...
			"Not able to setband!!!!");
				...
				case EVENT_COMPLETE_SCAN_PROGRESS:
                    ...
                    Log.i(TAG, "enter handleMessage ----EVENT_COMPLETE_SCAN_PROGRESS progress"
                            + progress);
					break;
			...
			case EVENT_TUNE_COMPLETE:
				Log.i(TAG, "enter handleMessage ----EVENT_TUNE_COMPLETE");
				Integer tuneFreq = (Integer) msg.obj;
                    Log.i(TAG, "enter handleMessage ----EVENT_TUNE_COMPLETE tuneFreq" + tuneFreq);
				...
				txtPsText.setText(null);
                    ...
                    Log.d(TAG, "sdefaultSettingOn" + sdefaultSettingOn +"mRdsState"+mRdsState);

				/*
				 * Enable the Audio routing after the tune complete , when FM
                     * FM app is started for the first time or after reentering
                     * the Fm app
				 */
				if (sdefaultSettingOn == false) {
					mStatus = sFmRadio.rxEnableAudioRouting();
					if (mStatus == false) {
                            showAlert(getParent(), "FmRadio", "Not able to enable audio!!!!");
                        }
			...
			showAlert(getParent(), "FmRadio", "Cannot enable RDS!!!!");
			}
			}
			...
			case EVENT_RDS_TEXT:
				...
				String rds = (String) msg.obj;
					Log.i(TAG, "enter handleMessage ----EVENT_RDS_TEXT rds"
							+ rds);
				sb = new StringBuilder("[RDS] ");
				sb.append(rds);
					Log.i(TAG, "enter handleMessage ----EVENT_RDS_TEXT sb.toString()"
							+ sb.toString());
			...
			case EVENT_PS_CHANGED:
				...
				sbPs.append(ps);
				Log.i(TAG, "enter handleMessage ----EVENT_PS_CHANGED sbPs.toString()"
						+ sbPs.toString());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case MENU_PRESET:
			...
			Intent i = new Intent(INTENT_PRESET);
                startActivityForResult(i, ACTIVITY_PRESET);
			break;
		case MENU_EXIT:
			/*
			 * The exit from the FM application happens here The audio will be
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
            ...
            if (fmAction.equals(FmRadioIntent.SET_MODE_MONO_STEREO_ACTION)) {
                Log.i(TAG, "enter onReceive SET_MODE_MONO_STEREO_ACTION " + fmAction);
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_MONO_STEREO_CHANGE, 0));
            }
            if (fmAction.equals(FmRadioIntent.DISPLAY_MODE_MONO_STEREO_ACTION)) {
                Log.i(TAG, "enter onReceive DISPLAY_MODE_MONO_STEREO_ACTION " + fmAction);
                Integer modeDisplay = intent.getIntExtra(FmRadioIntent.MODE_MONO_STEREO, 0);
                mHandler
                        .sendMessage(mHandler.obtainMessage(EVENT_MONO_STEREO_DISPLAY, modeDisplay));
            }
            ...
            if (fmAction.equals(FmRadioIntent.RDS_TEXT_CHANGED_ACTION)) {
                ...
                if (FM_SEND_RDS_IN_BYTEARRAY == true) {
                    ...
                    mHandler
                            .sendMessage(mHandler.obtainMessage(EVENT_RDS_TEXT, status, 0, rdsText));
                } else {
            ...
            if (fmAction.equals(FmRadioIntent.COMPLETE_SCAN_PROGRESS_ACTION)) {
                ...
                mHandler
                        .sendMessage(mHandler.obtainMessage(EVENT_COMPLETE_SCAN_PROGRESS, progress));
            ...
            if (fmAction.equals(FmRadioIntent.GET_CHANNEL_SPACE_ACTION)) {
                ...
                mHandler.sendMessage(mHandler
                        .obtainMessage(EVENT_GET_CHANNEL_SPACE_CHANGE, chSpace));
            }
            ...
            if (fmAction.equals(FmRadioIntent.GET_RDS_AF_SWITCH_MODE_ACTION)) {
                ...
                mHandler.sendMessage(mHandler
                        .obtainMessage(EVENT_GET_RDS_AF_SWITCHMODE, switchMode));
            }
            ...
            if (fmAction.equals(FmRadioIntent.GET_MONO_STEREO_MODE_ACTION)) {
                ...
                Long gMode = intent.getLongExtra(FmRadioIntent.GET_MODE, 0);
            ...
            if (fmAction.equals(FmRadioIntent.GET_MUTE_MODE_ACTION)) {
                ...
                Long gMuteMode = intent.getLongExtra(FmRadioIntent.GET_MUTE_MODE, 0);
            ...
            if (fmAction.equals(FmRadioIntent.GET_RSSI_THRESHHOLD_ACTION)) {
                ...
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_GET_RSSI_THRESHHOLD,
                        gRssiThreshhold));
            }
            ...
            if (fmAction.equals(FmRadioIntent.SET_DEEMP_FILTER_ACTION)) {
                Log.i(TAG, "enter onReceive SET_DEEMP_FILTER_ACTION " + fmAction);
            ...
            if (fmAction.equals(FmRadioIntent.PS_CHANGED_ACTION)) {
                ...
                if (FM_SEND_RDS_IN_BYTEARRAY == true) {
                    ...
                    mHandler.sendMessage(mHandler
                            .obtainMessage(EVENT_PS_CHANGED, status, 0, psName));
                } else {
            ...
            if (fmAction.equals(FmRadioIntent.SET_RSSI_THRESHHOLD_ACTION)) {
                Log.i(TAG, "enter onReceive SET_RSSI_THRESHHOLD_ACTION " + fmAction);
            ...
            if (fmAction.equals(FmRadioIntent.SET_RF_DEPENDENT_MUTE_ACTION)) {
                Log.i(TAG, "enter onReceive SET_RF_DEPENDENT_MUTE_ACTION " + fmAction);
            ...
            if (fmAction.equals(FmRadioIntent.COMPLETE_SCAN_DONE_ACTION)) {
                Log.i(TAG, "enter onReceive COMPLETE_SCAN_DONE_ACTION " + fmAction);
                ...
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_COMPLETE_SCAN_DONE, status,
                        noOfChannels, channelList));
            }

            if (fmAction.equals(FmRadioIntent.COMPLETE_SCAN_STOP_ACTION)) {
                Log.i(TAG, "enter onReceive COMPLETE_SCAN_STOP_ACTION " + fmAction);
                Bundle extras = intent.getExtras();
                int status = extras.getInt(FmRadioIntent.STATUS, 0);
                int channelValue = extras.getInt(FmRadioIntent.LAST_SCAN_CHANNEL, 0);
                Log
                        .i(TAG, "Last Scanned Channel Frequency before calling Stop Scan"
                                + channelValue);
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_COMPLETE_SCAN_STOP, status,
                        channelValue));
            }

            if (fmAction.equals(FmRadioIntent.MASTER_VOLUME_CHANGED_ACTION)) {
                Log.i(TAG, "enter onReceive MASTER_VOLUME_CHANGED_ACTION " + fmAction);
                mVolume = intent.getIntExtra(FmRadioIntent.MASTER_VOLUME, 0);
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_MASTER_VOLUME_CHANGED, mVolume));
            }
