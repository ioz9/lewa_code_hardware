...
void FM_TX_SM_TaskEventCb(FmcOsEvent osEvent)
{
  ...
  if ( osEvent&FMC_OS_EVENT_TIMER_EXPIRED)
    {
    ...
    if (osEvent & FMC_OS_EVENT_DISABELING)
                {
   ...
   if(osEvent &  FMC_OS_EVENT_GENERAL)
            {
                _FM_TX_SM_ProcessEvents();           
            }

     if (osEvent & FMC_OS_EVENT_INTERRUPT_RECIEVED)
    {
                                                            ...
                                                            void *eventData)
{
    ...
    /*In order to call fm_tx_Destroy in the same context , we must remove the disable 
        Command before sending Disable event to the application */
    if(currCmd.cmdType==FM_TX_CMD_DISABLE)
    {
    ...
    else
    {
       ...
       FMC_LOG_ERROR(("_FM_TX_SM_HandlerStartTransmission_Complete"));
    /* Notify the appliction about completion of the command */
    _FM_TX_SM_SendAppCmdCompleteEvent(  currCmd.context, 
                                            cmdCompletionStatus, 
                                            currCmd.cmdType);
